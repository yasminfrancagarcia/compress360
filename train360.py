...
class RateDistortionLoss(nn.Module):
    ...
    def __init__(self, lmbda=1e-2, type="mse"):
        ...
        self.lmbda = lmbda
        self.type = type
        self._weight_cache = {}  # para evitar recalcular o mapa de pesos toda iteração

    def _get_latitude_weights(self, H, W, device, dtype):
        """
        Retorna um tensor (1, 1, H, W) com o peso cos(phi) de cada linha,
        normalizado para média 1 (assim a escala do loss não muda em
        relação à MSE original, só a distribuição espacial do erro).
        """
        key = (H, W, device, dtype)
        if key not in self._weight_cache:
            j = torch.arange(H, device=device, dtype=dtype)
            #equação 8, calcula os pesos 
            w_row = torch.cos((j + 0.5 - H / 2.0) * math.pi / H)
            w_row = w_row.clamp(min=0)  # segurança numérica perto dos polos
            w_row = w_row / w_row.mean()  # normaliza para média 1
 
            weights = w_row.view(1, 1, H, 1).expand(1, 1, H, W).contiguous()
    ...
    def forward(self, output, target):
        ...
        if self.type == "mse":
            weights = self._get_latitude_weights(H, W, target.device, target.dtype)
            sq_err = (output["x_hat"] - target) ** 2  # (N, C, H, W)
            weighted_sq_err = sq_err * weights  # broadcast em N, C, W
 
            # Como weights tem média 1, essa média já é a WMSE (eq. 11):
            # sum(err^2 * w) / sum(w)  ==  mean(err^2 * w)  quando mean(w)=1

            out["mse_loss"] = weighted_sq_err.mean()
...
#     x_hat = x + 0.01 * torch.randn(B, C, H, W)
#     fake_output = {
#     "x_hat": x_hat,
#     "likelihoods": {"y": torch.rand(B, 32, H // 16, W // 16).clamp(1e-6, 1)},
#         }
#     criterion = RateDistortionLoss(lmbda=0.01, type="mse")
#     out = criterion(fake_output, x)
